...
import com.search.domain.websearch.infrastructure.KakaoWebRepositoryImpl
...
class WebQueryServiceItTest extends Specification {
    ...
    @SpringBean
    NaverApiKeyManagerIfs naverApiKeyManagerIfs = Mock(NaverApiKeyManagerIfs)
    ...
    def "정상상황에서는 Circuit의 상태가 CLOSED이고 naver쪽으로 호출이 들어간다."(){
        ...
        then:
        1 * naverWebRepository.search(keyword, page, size) >> new WebSearchPageResult<WebSearchResult>(1, 10, 0, [])
    ...
    def "circuit이 open되면 kakao쪽으로 요청을 한다."() {
        ...
        def page = 1
        def size = 10
        def kakaoResponse = new WebSearchPageResult<WebSearchResult>(1, 10, 0, [])
        ...
        naverWebRepository.search(keyword, page, size) >> {throw new RuntimeException("Naver API ERROR")}
