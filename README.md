...
또한, 외부 API 연동 시 API 호출 비용 최적화 및 장애 대응을 중요하게 고려하였습니다.  

특히 OpenAI (GPT)를 백엔드 서버에서 직접 연동할 경우 기능에 따른 서버의 비즈니스 로직이 비대해져 유지보수성의 어려움을 느꼈습니다. 

따라서 좀 더 효율적인 방식을 고민하였고, 워크플로우(N8N) 서비스를 통해 이미 가공된 데이터 결과만을 가지고 서버와 통신하는 방법을 채택하였습니다.
