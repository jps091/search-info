...
또한 외부 API 연동 시 API 호출 비용 최적화 및 장애 대응을 중요하게 고려하였고, 특히 OpenAI(GPT)를 백엔드 서버에서 직접 연동할 경우 기능에 따른 서버의 로직이 비대해지는 문제를 어떻게 해결할지 고민하였습니다.
...
- 성능 최적화
  ...
  - 12:00 ~ 14:00 요청량이 평소의 3배 이상으로 급증하는 피크 타임입니다.
    
- 웹 검색 기능
  - 키워드로 검색을 시도하면 해당 키워드 관련 자료를 제공해야 한다.
  - Naver API 장애가 발생해도, 서비스가 정상 제공되어야 한다.
  - 오전 9시마다 배너에 금일 IT 뉴스 3개를 제공해야 한다. 
...
다양한 워크플로우 서비스(Airflow, Zapier 등) 중에서 **n8n**을 선택한 이유는, AWS EC2 인스턴스(t2.micro) 환경에서도 구동 가능한 **경량화된 도커 이미지의 제공**으로 인한 설치의 용이성, 그리고 **서버 자체에 구축 시 과금 없이 운영이 가능하다는 비용적 장점** 때문입니다.
...
Open API 연동에서는 API 호출 비용을 최적화하고 장애 대응을 고려한 설계(서킷브레이커)를 통해 실무에서 발생할 수 있는 상황들을 경험할 수 있었고, OpenAI(GPT) 연동 과정에서는 워크플로우(N8N) 서비스를 통해 이미 가공된 데이터 결과만을 가지고 서버와 통신하는 방법을 채택하였습니다.

그리고 Stomp를 활용한 실시간 채팅 기능을 구현하면서 Http, SSE, WebSocket 프로토콜 차이와 사용법을 학습하여 실제 서비스를 구축할 수 있었습니다.
